...
# Returns a newline seperated UFT-8 encoded string of all windows for bemenu
def build_bemenu_string(windows):
...
# Returns the sway window id of the window that was selected by the user inside bemenu
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Bemenu based window switcher")

    windows = get_windows()
    
    parsed_windows = parse_windows(windows)    
    
    bemenu_string = build_bemenu_string(parsed_windows)

    selected = show_bemenu(bemenu_string)
